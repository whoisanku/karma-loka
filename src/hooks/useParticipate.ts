...
export function useParticipate(roomId: string | number, stakeAmount: string): UseParticipateReturn {
  ...
  const [participateTxHash, setParticipateTxHash] = useState<`0x${string}` | null>(null);
  const [pendingParticipate, setPendingParticipate] = useState(false);

  const { address, isConnected } = useAccount();
  const { connect, connectors, isPending: isConnecting } = useConnect();

  const stakeAmountWei = stakeAmount && Number(stakeAmount) > 0 ? parseUnits(stakeAmount, 6) : 0n;
  ...
  const { isSuccess: isApprovalSuccess, error: approvalReceiptError } = useWaitForTransactionReceipt({
    hash: approvalHash,
    confirmations: 1,
    query: { enabled: !!approvalHash },
  });

  const { isSuccess: isParticipateSuccess, error: participateReceiptError } = useWaitForTransactionReceipt({
  ...
  const handleParticipate = useCallback(async () => {
    ...
    if (!writeParticipate) {
      ...
      setErrorMessage('Participation not available. Please refresh and try again.');
      return;
    }
    ...
    try {
      ...
      setErrorMessage(error instanceof Error ? error.message : 'Participation failed');
    }
  ...
  }, [isApprovalSuccess, approvalHash, pendingParticipate, handleParticipate, refetchAllowance]);
  ...
  useEffect(() => {
    const error = approvalError || participateError || approvalReceiptError || participateReceiptError;
    if (error) {
      console.error('Transaction error:', error);
      setCurrentStep('error');
      setErrorMessage(error instanceof Error ? error.message : 'Transaction failed');
    }
  }, [approvalError, participateError, approvalReceiptError, participateReceiptError]);
  ...
  const handleApproveUSDC = useCallback(async () => {
    ...
    try {
      console.log('Requesting approval for stake:', formatUnits(stakeAmountWei, 6));
